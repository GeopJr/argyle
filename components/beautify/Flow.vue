...
import PrettierFlow from 'prettier/parser-flow'
...
export default {
  ...
  methods: {
    beautify () {
      let format
      try {
        format = Prettier.format(this.input, { semi: false, parser: 'flow', plugins: [PrettierFlow] })
      } catch {
        format = 'This is not valid Flow'
      }
